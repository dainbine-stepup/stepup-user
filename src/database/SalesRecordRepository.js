...
const SalesRecordRepository = {
    ...
    insertSalesRecord: (params, successCallback, errorCallback) => {
        ...
        if (insertPeriodType === 'month') {
            ...
            const now = new Date();
            const modDt = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}-${String(now.getDate()).padStart(2, '0')} ${String(now.getHours()).padStart(2, '0')}:${String(now.getMinutes()).padStart(2, '0')}:${String(now.getSeconds()).padStart(2, '0')}`;
            ...
            db.transaction((tx) => {
                dates.forEach((date, index) => {
                    const amount = index === 0 ? perDayAmount + adjustment : perDayAmount;
                    // 먼저 UPDATE 시도
                    tx.executeSql(
    ...
    getSalesRecord: (recordPeriodType, successCallback, errorCallback) => {
        ...
        // 주간 합계 매출 조회
            ...
            const query = `
                SELECT
                    date(sales_date, 'weekday 1') || ' ~ ' || date(sales_date, 'weekday 0') AS sales_date,
    ...
    // 수정
    updateSalesAmount: (recordPeriodType, updateAmount, dates, successCallback, errorCallback) => {
        const now = new Date();
        const modDt = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}-${String(now.getDate()).padStart(2, '0')} ${String(now.getHours()).padStart(2, '0')}:${String(now.getMinutes()).padStart(2, '0')}:${String(now.getSeconds()).padStart(2, '0')}`;
    ...
    // 삭제 (비활성화)
    deleteSalesRecords: (recordPeriodType, dates, successCallback, errorCallback) => {
        const disabledCd = 'STTCD002';
        const statusCd = 'STTCD001';
